...
public class Crop : MonoBehaviour
{
    ...
    IEnumerator GrowCrop()
    {
        while(maxGrowRange != curGrowRange)
        {
            ...
            if (canGrow)
            {
                if ( (this.gameObject.transform.parent.gameObject.transform.parent.gameObject.transform.parent.GetComponent<Soil>().watering > 80 && this.gameObject.transform.parent.gameObject.transform.parent.gameObject.transform.parent.GetComponent<Soil>().watering < 30))
                {
                    curGrowRange = -1;
                }
                switch (Name)
